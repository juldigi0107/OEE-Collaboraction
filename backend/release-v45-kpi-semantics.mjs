...
async function maintenanceMetrics(env,baseMetrics,gov){
 ...
 const coverage=await one(env.DB,"SELECT SUM(CASE WHEN module='checklist' AND deleted=0 THEN 1 ELSE 0 END) checklist,SUM(CASE WHEN module='logbook' AND deleted=0 THEN 1 ELSE 0 END) logbook FROM entries")||{};
 ...
 setMetric(metrics,'pm_compliance',{label:'PM compliance',value:null,unit:'',source:'Coverage / readiness',note:'Belum dihitung: D1/source ZIP belum memiliki grain PM plan vs PM completion authoritative. Corrective ODIN tidak dipakai sebagai pengganti PM.'});
 setMetric(metrics,'predictive_patrol',{label:'Predictive patrol completion',value:null,unit:'',source:'Coverage / readiness',note:`Belum dihitung: Patrol Web sumber belum memiliki transaksi patrol authoritative. ${Number(coverage.checklist||0)} checklist dan ${Number(coverage.logbook||0)} logbook terdaftar tidak diasumsikan sebagai denominator patrol.`});
 setMetric(metrics,'repeat_failure',{label:'Repeat failure rate',value:null,unit:'',source:'Coverage / readiness',note:'Belum dihitung: definisi repeat window, failure taxonomy/grain, dan rule same-failure belum disahkan. Pengulangan notifikasi tidak otomatis dianggap repeat failure.'});
 setMetric(metrics,'abnormality_closure',{label:'Abnormality closure lead time',value:null,unit:'',source:'Coverage / readiness',note:'Belum dihitung: abnormality patrol membutuhkan detect timestamp, owner, due date, verified closure, dan evidence yang konsisten.'});
 setMetric(metrics,'permanent_action_verification',{label:'Permanent action verification',value:null,unit:'',source:'Coverage / readiness',note:'Belum dihitung: Maintenance Close/corrective action tidak disamakan dengan permanent-action verification tanpa verification gate dan evidence khusus.'});
