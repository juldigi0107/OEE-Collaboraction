...
async function validateDependencies(env,key,value){
 if(key==='DATA_GOVERNANCE.source_authority'&&value.approved===true){const domains=value.domains||{};for(const domain of ['production','quality','maintenance','ppic','development','master']){const raw=domains[domain],id=clean(typeof raw==='string'?raw:raw?.source_id);if(!id||!await one(env.DB,'SELECT id FROM sources WHERE id=?',id))return `Source Authority ${domain}: source ID tidak ditemukan pada D1`;}}
