...
async function validateReleased(env,p,fgUnit=null){const problem=baseProblem(p);if(problem)return problem;if(clean(p?.status)!=='Released')return '';const unit=clean(p?.unit).toLowerCase()||fgUnit??await approvedFgUnit(env);if(!clean(unit))return 'Planning Released wajib memiliki Satuan target atau FG Unit authoritative yang sudah disahkan';if(!/^[a-z0-9%/._ -]{1,24}$/i.test(unit))return 'Satuan target Planning Released tidak valid';return '';}
