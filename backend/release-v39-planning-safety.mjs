...
export async function handlePlanningSafetyV39(req,env){
 ...
 if(method==='GET'&&path==='/api/shopfloor/plans'){
  ...
  const rows=await all(env.DB,"SELECT id,payload,version,updated FROM entries WHERE module='planning' AND deleted=0 AND json_extract(payload,'$.status')='Released' ORDER BY json_extract(payload,'$.date'),updated LIMIT 200");
  return response(req,env,rows.filter(row=>!validateReleased(J(row.payload,{}))));
 }
 if(method==='POST'&&path==='/api/shopfloor/start'){
  const u=await auth(req,env);if(!u||!allowProd(u,'create'))return null;if(await passwordGate(env,u))return error(req,env,403,'Ganti password awal terlebih dahulu');let body;try{body=await req.clone().json();}catch{return null;}const plan=await one(env.DB,"SELECT payload FROM entries WHERE id=? AND module='planning' AND deleted=0",body.plan_id||'');if(!plan)return null;const problem=validateReleased(J(plan.payload,{}));return problem?error(req,env,409,problem):null;
 }
 if(method==='POST'&&path==='/api/entries'){
  let body;try{body=await req.clone().json();}catch{return null;}if(body?.module!=='planning')return null;const u=await auth(req,env);if(!u||!allowPpic(u,'create'))return null;const problem=validateReleased(body.payload);return problem?error(req,env,400,problem):null;
 }
 if(method==='PUT'&&path.startsWith('/api/entries/')){
  const id=decodeURIComponent(path.slice('/api/entries/'.length)),entry=await one(env.DB,'SELECT module FROM entries WHERE id=? AND deleted=0',id);if(!entry||entry.module!=='planning')return null;const u=await auth(req,env);if(!u||!allowPpic(u,'update'))return null;let body;try{body=await req.clone().json();}catch{return null;}const problem=validateReleased(body.payload);return problem?error(req,env,400,problem):null;
 }
 return null;
}
