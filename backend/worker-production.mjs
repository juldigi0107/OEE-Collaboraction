...
export default {
 async fetch(req,env,ctx){
  ...
  const machineGovernanceResponse=await handleMachineGovernanceV20(req,env);if(machineGovernanceResponse)return machineGovernanceResponse;
 ...
 scheduled(controller,env,ctx){const existing=app.scheduled?.(controller,env,ctx);ctx.waitUntil(runLifecycleHousekeepingV46(env));ctx.waitUntil(backfillLiveRegistersV49(env,50));return existing;}
};
