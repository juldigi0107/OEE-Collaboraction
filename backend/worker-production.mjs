...
export default {
 async fetch(req,env,ctx){
  ...
  const machineGovernanceResponse=await handleMachineGovernanceV20(req,env);if(machineGovernanceResponse){if(machineGovernanceResponse.ok){if(workflowSignal)ctx.waitUntil(afterWorkflowLineageV50(workflowSignal,machineGovernanceResponse.clone(),env));if(telemetryStartSignal)ctx.waitUntil(afterTelemetryStartV61(telemetryStartSignal,machineGovernanceResponse.clone(),env));if(workCalendarSignal)ctx.waitUntil(afterWorkCalendarStartV62(workCalendarSignal,machineGovernanceResponse.clone(),env));}return machineGovernanceResponse;}
