...
import {handleHmiOeeV81} from './release-v81-hmi-oee.mjs';
import {handleFieldDisplayV82} from './release-v82-field-display.mjs';
import {handleDisplayDeviceV83} from './release-v83-display-device.mjs';
...
async function ensureAdditiveSchema(env){if(!schemaReady){schemaReady=(async()=>{await env.DB.prepare('CREATE TABLE IF NOT EXISTS asset_catalog(id TEXT PRIMARY KEY,parent TEXT NOT NULL,path TEXT NOT NULL)').run();await env.DB.prepare('CREATE INDEX IF NOT EXISTS asset_parent ON asset_catalog(parent)').run();await env.DB.prepare("CREATE TABLE IF NOT EXISTS display_pair_codes(code_hash TEXT PRIMARY KEY,display_id TEXT NOT NULL,machine_code TEXT NOT NULL,created_by TEXT NOT NULL,created_ts INTEGER NOT NULL,expires_ts INTEGER NOT NULL,used_ts INTEGER)").run();await env.DB.prepare("CREATE TABLE IF NOT EXISTS display_devices(id TEXT PRIMARY KEY,display_id TEXT NOT NULL,machine_code TEXT NOT NULL,name TEXT NOT NULL,token_hash TEXT NOT NULL UNIQUE,active INTEGER NOT NULL DEFAULT 1,created_by TEXT NOT NULL,created_ts INTEGER NOT NULL,expires_ts INTEGER NOT NULL,last_seen_ts INTEGER)").run();await addColumnIfMissing(env,'quality_events','unit','ALTER TABLE quality_events ADD COLUMN unit TEXT');await addColumnIfMissing(env,'production_runs','unit','ALTER TABLE production_runs ADD COLUMN unit TEXT');await addColumnIfMissing(env,'production_runs','plan_id','ALTER TABLE production_runs ADD COLUMN plan_id TEXT');await addColumnIfMissing(env,'production_runs','counter_start_trusted','ALTER TABLE production_runs ADD COLUMN counter_start_trusted INTEGER NOT NULL DEFAULT 0');await addColumnIfMissing(env,'production_runs','work_date','ALTER TABLE production_runs ADD COLUMN work_date TEXT');await addColumnIfMissing(env,'production_runs','nc_qty','ALTER TABLE production_runs ADD COLUMN nc_qty REAL');await addColumnIfMissing(env,'production_runs','quality_rule','ALTER TABLE production_runs ADD COLUMN quality_rule TEXT');await addColumnIfMissing(env,'production_runs','governance_approved','ALTER TABLE production_runs ADD COLUMN governance_approved INTEGER NOT NULL DEFAULT 0');await addColumnIfMissing(env,'production_runs','governance_updated_at','ALTER TABLE production_runs ADD COLUMN governance_updated_at TEXT');await addColumnIfMissing(env,'maintenance_calls','resolution_note','ALTER TABLE maintenance_calls ADD COLUMN resolution_note TEXT');const indexes=['CREATE INDEX IF NOT EXISTS sessions_expires ON sessions(expires)','CREATE INDEX IF NOT EXISTS login_attempts_until ON login_attempts(until_ts)','CREATE INDEX IF NOT EXISTS entries_module_updated ON entries(module,deleted,updated DESC)','CREATE INDEX IF NOT EXISTS production_runs_start ON production_runs(start_ts DESC)','CREATE INDEX IF NOT EXISTS production_runs_plan ON production_runs(plan_id)','CREATE INDEX IF NOT EXISTS downtime_class_start ON downtime_events(class,start_ts DESC)','CREATE INDEX IF NOT EXISTS maintenance_calls_requested ON maintenance_calls(requested_ts DESC)','CREATE INDEX IF NOT EXISTS quality_events_created ON quality_events(created_ts DESC)','CREATE INDEX IF NOT EXISTS approvals_type_status_requested ON approvals(entity_type,status,requested_ts DESC)','CREATE INDEX IF NOT EXISTS integration_sync_connection ON integration_sync_log(connection_id,started_ts DESC)','CREATE INDEX IF NOT EXISTS display_devices_layout ON display_devices(display_id,active,expires_ts)','CREATE INDEX IF NOT EXISTS display_pair_expiry ON display_pair_codes(expires_ts,used_ts)'];for(const sql of indexes)await env.DB.prepare(sql).run();})().catch(error=>{schemaReady=null;throw error;});}return schemaReady;}
...
export default {
 async fetch(req,env,ctx){
  ...
  const displayDeviceResponse=await handleDisplayDeviceV83(req,env);if(displayDeviceResponse)return displayDeviceResponse;
  ...
  const hmiOeeResponse=await handleHmiOeeV81(req,env);if(hmiOeeResponse){if(hmiOeeResponse.ok){await settleCritical('hmi-oee-lineage',[approvalSignal&&afterReleaseV11(approvalSignal,hmiOeeResponse.clone(),req,env),workflowSignal&&afterWorkflowLineageV50(workflowSignal,hmiOeeResponse.clone(),env),liveSignal&&afterLiveRegisterV49(liveSignal,hmiOeeResponse.clone(),env)]);}return hmiOeeResponse;}
  ...
  const patrolResponse=await handlePatrolV94(req,env);if(patrolResponse)return patrolResponse;
  ...
  const fieldDisplayResponse=await handleFieldDisplayV82(req,env);if(fieldDisplayResponse)return fieldDisplayResponse;
  ...
  const attentionCoverageResponse=await handleAttentionCoverageV85(req,env);if(attentionCoverageResponse)return attentionCoverageResponse;
