...
async function qualityPost(req,env,u){
 ...
 return out(req,env,{ok:true,id,unit,classification_complete:Math.abs(good+reject-sample)<=1e-9,verification:'PENDING'});
}
async function qcDashboard(req,env,u,url){
 ...
 const complete="ABS(COALESCE(good_qty,0)+COALESCE(reject_qty,0)-COALESCE(sample_qty,0))<=0.000001";
 const groups=await all(env.DB,`SELECT COALESCE(NULLIF(lower(trim(unit)),''),'__missing__') unit,COUNT(*) events,COALESCE(SUM(sample_qty),0) sample,COALESCE(SUM(good_qty),0) good,COALESCE(SUM(reject_qty),0) reject,SUM(CASE WHEN ${complete} THEN 1 ELSE 0 END) complete_events,SUM(CASE WHEN ${complete} THEN COALESCE(sample_qty,0) ELSE 0 END) classified_sample,SUM(CASE WHEN ${complete} THEN COALESCE(good_qty,0) ELSE 0 END) classified_good,SUM(CASE WHEN ${complete} THEN 0 ELSE 1 END) incomplete_events FROM quality_events WHERE created_ts>=datetime('now','-30 days') GROUP BY COALESCE(NULLIF(lower(trim(unit)),''),'__missing__') ORDER BY unit`),total=groups.reduce((n,x)=>n+Number(x.events||0),0),metrics=[metric('events','Quality event 30 hari',total,'','HMI Quality','Jumlah event; kuantitas tidak dijumlahkan lintas satuan')];
 let known=0,missing=0;for(const g of groups){const events=Number(g.events||0);if(g.unit==='__missing__'){missing+=events;continue;}known++;const key=String(g.unit).replace(/[^a-z0-9]+/g,'_'),sample=Number(g.sample||0),reject=Number(g.reject||0),classifiedSample=Number(g.classified_sample||0),classifiedGood=Number(g.classified_good||0),incomplete=Number(g.incomplete_events||0),rate=classifiedSample>0?classifiedGood/classifiedSample:null;metrics.push(metric('sample_'+key,'Sample · '+g.unit,sample,g.unit,'HMI Quality','30 hari · hanya unit yang sama'),metric('reject_'+key,'Reject · '+g.unit,reject,g.unit,'HMI Quality','30 hari · hanya unit yang sama'),metric('quality_'+key,'Quality rate · '+g.unit,rate,'ratio','HMI Quality','Good / sample hanya dari event dengan Good + Reject = Sample'));if(incomplete)metrics.push(metric('incomplete_'+key,'Sampling belum lengkap · '+g.unit,incomplete,'event','HMI Quality','Tidak dimasukkan ke Quality Rate sampai seluruh sample diklasifikasikan'));}
 ...
 return out(req,env,{department:'QC',generated_at:now(),metrics,quality_rate_policy:'Rate live hanya menggunakan event dengan klasifikasi lengkap dalam unit yang sama.'});
}
async function approvalList(req,env){
 ...
 const qualityMap=new Map(),runMap=new Map();if(qualityIds.length){const placeholders=qualityIds.map(()=>'?').join(','),unitRows=await all(env.DB,`SELECT id,lower(trim(unit)) unit,sample_qty,good_qty,reject_qty FROM quality_events WHERE id IN (${placeholders})`,...qualityIds);unitRows.forEach(x=>qualityMap.set(x.id,{unit:clean(x.unit),classification_complete:Math.abs(Number(x.good_qty||0)+Number(x.reject_qty||0)-Number(x.sample_qty||0))<=1e-6}));}if(runIds.length){const placeholders=runIds.map(()=>'?').join(','),unitRows=await all(env.DB,`SELECT id,lower(trim(unit)) unit,nc_qty,quality_rule,governance_approved,governance_updated_at,actual_qty,good_qty,reject_qty FROM production_runs WHERE id IN (${placeholders})`,...runIds);unitRows.forEach(x=>{const rule=clean(x.quality_rule)||'good_total',nc=x.nc_qty===null||x.nc_qty===undefined?null:Number(x.nc_qty),ncForBalance=nc??0,complete=Math.abs(Number(x.good_qty||0)+Number(x.reject_qty||0)+ncForBalance-Number(x.actual_qty||0))<=1e-6&&(rule!=='good_nc_total'||nc!==null);runMap.set(x.id,{unit:clean(x.unit),nc_qty:nc,quality_rule:rule,governance_approved:Number(x.governance_approved||0)===1,governance_updated_at:x.governance_updated_at||null,classification_complete:complete});});}
 for(const a of rows){if(a.entity_type==='quality'){const q=qualityMap.get(a.entity_id)||{unit:'',classification_complete:false};a.entity={...(a.entity||{}),unit:q.unit,unit_status:q.unit?'known':'legacy_missing',classification_complete:q.classification_complete};}if(a.entity_type==='production_run'){const r=runMap.get(a.entity_id)||{unit:'',nc_qty:null,quality_rule:'good_total',governance_approved:false,governance_updated_at:null,classification_complete:false};a.entity={...(a.entity||{}),unit:r.unit,unit_status:r.unit?'known':'legacy_missing',nc_qty:r.nc_qty,quality_rule:r.quality_rule,quality_rule_label:r.quality_rule==='good_nc_total'?'(Good + NC) / Total':'Good / Total',governance_approved:r.governance_approved,governance_updated_at:r.governance_updated_at,classification_complete:r.classification_complete};}}
