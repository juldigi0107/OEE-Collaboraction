...
async function lineageRows(env,limit=500){return all(env.DB,"SELECT r.id run_id,r.plan_id,r.status run_status,r.start_ts,r.end_ts,e.payload plan_payload,e.version plan_version,a.status approval_status,a.decided_ts,a.note approval_note FROM production_runs r LEFT JOIN entries e ON e.id=r.plan_id AND e.module='planning' AND e.deleted=0 LEFT JOIN approvals a ON a.entity_type='production_run' AND a.entity_id=r.id AND a.step='FINAL_VERIFY' WHERE trim(COALESCE(r.plan_id,''))<>'' ORDER BY COALESCE(r.end_ts,r.start_ts) DESC LIMIT ?",Math.max(1,Math.min(1000,Number(limit)||500)));}
...
export async function workflowHealthV51(env){
 ...
 return {generated_at:new Date().toISOString(),ready:orphaned===0&&mismatches===0,linked_runs:linked,checked_runs:rows.length,lineage_mismatches:mismatches,orphaned_plan_links:orphaned,multi_run_plans:multi,pending_production_verification:pending,note:'Multi-run plan dilaporkan untuk audit, tetapi tidak otomatis dianggap error karena rerun terkontrol dapat mempertahankan lineage plan yang sama.'};
}
