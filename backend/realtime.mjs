...
export async function handleRealtime(req,env){
 ...
 try{
  ...
  const u=await authUser(req,env);if(!u)fail(401,'Silakan login kembali');const pf=await one(db,'SELECT must_change FROM password_flags WHERE user_id=?',u.id);if(pf?.must_change)fail(403,'Ganti password awal terlebih dahulu');
  ...
  if(path==='/api/shopfloor/start'&&method==='POST'){
   if(!allow(u,'PROD','create'))fail(403,'Akun ini hanya dapat melihat HMI');const b=await req.json(),code=cleanCode(b.machine),mid=machineId(code);if(!code||!b.pro)fail(400,'Mesin dan PRO wajib dipilih');const plan=await one(db,"SELECT * FROM entries WHERE id=? AND module='planning' AND deleted=0",b.plan_id||'');if(!plan)fail(409,'Pilih planning yang telah disiapkan PPIC');const pp=J(plan.payload,{});if(String(pp.pro)!==String(b.pro)||cleanCode(pp.machine)!==code||!['Direncanakan','Released'].includes(pp.status))fail(409,'Planning, mesin, PRO atau status tidak cocok');const checks=b.checklist||{};if(!['material','qc','safety','tools'].every(k=>checks[k]===true))fail(409,'Checklist material, QC, safety dan tools harus lengkap sebelum Start PRO');if(await one(db,"SELECT id FROM production_runs WHERE machine_id=? AND status='RUNNING'",mid))fail(409,'Mesin masih memiliki PRO aktif');
  ...
  if(path==='/api/approvals'&&method==='POST'){
   const b=await req.json();if(!allow(u,b.entity_type==='quality'?'QC':'PROD','create'))fail(403,'Tidak memiliki izin approval');if(!['production_run','downtime','quality'].includes(b.entity_type))fail(400,'Jenis approval tidak valid');const id=uid();await run(db,"INSERT INTO approvals(id,entity_type,entity_id,step,status,requested_by) VALUES(?,?,?,?, 'PENDING',?) ON CONFLICT(entity_type,entity_id,step) DO NOTHING",id,b.entity_type,b.entity_id,b.step||'VERIFY',u.id);return out({ok:true,id});
  }
  if(path==='/api/approvals/decide'&&method==='POST'){
   const b=await req.json();const a=await one(db,'SELECT * FROM approvals WHERE id=?',b.id);if(!a)fail(404,'Approval tidak ditemukan');if(!allow(u,a.entity_type==='quality'?'QC':'PROD','update'))fail(403,'Khusus approver department');if(!['APPROVED','REJECTED'].includes(b.status))fail(400,'Keputusan tidak valid');await run(db,'UPDATE approvals SET status=?,decided_by=?,decided_ts=?,note=? WHERE id=?',b.status,u.id,now(),b.note||null,b.id);return out({ok:true});
  }
