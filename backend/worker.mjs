...
const modules={confirmation:'PPIC',planning:'PPIC',production:'PROD',downtime:'PROD',quality:'QC',maintenance:'MTC',development:'PDS',checklist:'PROD',logbook:'PROD',process:'QC',energy:'PROD',master:'PROJECT',project:'PROJECT',batch:'PROD'};
export function validateEntry(module,p){
 if(!p||typeof p!=='object'||Array.isArray(p))fail(400,'Isi formulir tidak valid');
 if(JSON.stringify(p).length>40000)fail(413,'Transaksi terlalu besar');
 if(typeof p.title!=='string'||!p.title.trim())fail(400,'Judul wajib diisi');
 if(p.title.length>300)fail(400,'Judul terlalu panjang');
 for(const k of ['total','good','planned','runtime','speed','minutes','cost','lsl','usl',...(module==='process'?['value']:[]),'kwh','reject','qty','scrap','hours','progress'])if(p[k]!==undefined&&p[k]!==''&&!Number.isFinite(Number(p[k])))fail(400,k+' harus berupa angka');
 ...
 if(module==='batch'){for(const k of ['input_batch','output_batch','pro','machine','date','unit'])if(!p[k])fail(400,k+' wajib diisi');for(const k of ['qty','good','reject','nc'])if(p[k]===undefined||p[k]===''||!Number.isFinite(Number(p[k]))||Number(p[k])<0)fail(400,k+' harus angka tidak negatif');if(+p.good+(+p.reject)+(+p.nc)>+p.qty)fail(400,'Total hasil melebihi input batch');}
 ...
 if(p.date&&(!/^\d{4}-\d{2}-\d{2}$/.test(p.date)||!Number.isFinite(Date.parse(p.date))||new Date(p.date).toISOString().slice(0,10)!==p.date))fail(400,'Format tanggal harus YYYY-MM-DD');
 return p;
}
...
export default {async fetch(req,env){
 ...
 try{
  ...
  const audit=(u,a,id,b,c)=>db.prepare('INSERT INTO audit(id,user_id,action,entity_id,before_json,after_json) SELECT ?,?,?,?,?,? WHERE changes()>0').bind(uid(),u.id,a,id,b?JSON.stringify(b):null,c?JSON.stringify(c):null);
  ...
  const pf=await one('SELECT must_change FROM password_flags WHERE user_id=?',u.id);if(pf?.must_change)fail(403,'Ganti password awal terlebih dahulu');
  if(path==='/api/catalog')return json({sources:await all('SELECT * FROM sources ORDER BY department,name'),sheets:await all('SELECT * FROM sheets ORDER BY department,source_id,name'),settings:u.role==='superadmin'?await all('SELECT * FROM settings'):await all('SELECT * FROM settings WHERE department=?',u.department)});
  ...
  if(path==='/api/records'&&method==='GET'){
   ...
   if(!sheet)fail(400,'Sheet wajib dipilih');const filter=' WHERE deleted=0 AND (?=\'\' OR instr(lower(payload),lower(?))>0)';
  ...
  if(path.startsWith('/api/records/')&&['PUT','DELETE'].includes(method)){
   ...
   const version=Number(old.version)+1;const result=await db.batch([db.prepare('INSERT INTO records(id,sheet_id,department,row_num,payload,version,deleted) VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload,version=excluded.version,deleted=excluded.deleted WHERE records.version=excluded.version-1').bind(id,old.sheet_id,old.department,old.row_num,payload,version,deleted),audit(u,method==='DELETE'?'archive.delete':'archive.update',id,old,after)]);
   if(!result[0].meta.changes)fail(409,'Data telah berubah. Muat ulang.');return json({ok:true,version,warning:'Snapshot formula sumber tidak dihitung ulang otomatis; gunakan modul transaksi untuk kalkulasi baru.'});
  }
  ...
  if(path==='/api/entries'&&method==='GET'){
   ...
   return json({rows:await all('SELECT * FROM entries WHERE module=? AND deleted=0 AND instr(lower(payload),lower(?))>0 ORDER BY updated DESC LIMIT 50 OFFSET ?',module,q,page*50),total:(await one('SELECT count(*) n FROM entries WHERE module=? AND deleted=0 AND instr(lower(payload),lower(?))>0',module,q)).n,page});
  }
  ...
  if(path.startsWith('/api/entries/')&&['PUT','DELETE'].includes(method)){
   ...
   const result=await db.batch([db.prepare('UPDATE entries SET payload=?,deleted=?,version=version+1,updated=CURRENT_TIMESTAMP WHERE id=? AND version=?').bind(p?JSON.stringify(p):old.payload,method==='DELETE'?1:0,id,b.version),audit(u,'entry.'+method.toLowerCase(),id,old,p)]);if(!result[0].meta.changes)fail(409,'Data telah berubah. Muat ulang.');return json({ok:true});
  }
  ...
  if(path.startsWith('/api/files/')){
   const id=path.slice(11),meta=await one('SELECT f.* FROM source_files f JOIN sources s ON s.id=f.source_id WHERE f.source_id=?',id);if(!meta)fail(404,'File sumber tidak ditemukan di D1');let next=0;const batchSize=32;
