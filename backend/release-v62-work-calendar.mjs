...
export async function captureWorkCalendarStartV62(req,env){
 ...
 const cfg=await calendar(env),ctx=deriveContext(cfg);if(!ctx.approved||!ctx.runtime_ready||!ctx.shift)return null;let body={};try{body=await req.clone().json();}catch{}return {user_id:u.id,plan_id:clean(body.plan_id),work_date:ctx.work_date,shift:ctx.shift,timezone:ctx.timezone,local_time:ctx.local_time};
}
