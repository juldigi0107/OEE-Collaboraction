...
const sameFingerprint=(a,b)=>Array.isArray(a)&&Array.isArray(b)&&a.length===b.length&&a.every((x,i)=>x===b[i]);
async function releaseEvidence(env,buildVersion,releaseFingerprint=[]){
 const [signoffRow,snapshotRow]=await Promise.all([one(env.DB,"SELECT value FROM settings WHERE key='UAT_RELEASE.signoff'"),one(env.DB,"SELECT value FROM settings WHERE key='UAT_RELEASE.runtime_snapshot'")]),signoff=parse(signoffRow?.value,{}),snapshot=parse(snapshotRow?.value,{}),status=clean(signoff.status)||'not_started',hasSnapshot=!!clean(snapshot.captured_at),signedFingerprint=Array.isArray(snapshot?.build?.release_fingerprint)?snapshot.build.release_fingerprint:[],buildMatches=hasSnapshot&&clean(snapshot?.build?.version)===clean(buildVersion)&&sameFingerprint(signedFingerprint,releaseFingerprint),current=status==='passed'&&hasSnapshot&&snapshot.signoff_status==='passed'&&buildMatches;
 let staleReason=clean(signoff.stale_reason)||null;if(!current&&!staleReason){if(status!=='passed')staleReason='Final Sign-off belum aktif atau perlu dilakukan ulang.';else if(!hasSnapshot)staleReason='Final Sign-off belum memiliki runtime evidence snapshot.';else if(!buildMatches)staleReason='Build/fingerprint production berbeda dari build yang terakhir disahkan.';else staleReason='Runtime evidence terakhir tidak mewakili Final Sign-off aktif.';}
 return {status,has_snapshot:hasSnapshot,current,stale:hasSnapshot&&!current,stale_reason:staleReason,stale_at:signoff.stale_at||null,captured_at:snapshot.captured_at||null,signed_by:hasSnapshot?{name:snapshot?.signed_by?.name||null,username:snapshot?.signed_by?.username||null}:null,signed_build:hasSnapshot?{version:snapshot?.build?.version||null,fingerprint_count:signedFingerprint.length}:null,current_build:{version:clean(buildVersion)||null,fingerprint_count:Array.isArray(releaseFingerprint)?releaseFingerprint.length:0},build_matches:buildMatches};
}
...
export async function handleDataLifecycleV46(req,env,buildVersion,releaseFingerprint=[]){
 ...
 const base=await handleSupportV21(req,env,buildVersion,releaseFingerprint);if(!base||!base.ok)return base;let body;try{body=await base.clone().json();}catch{return base;}const [storage,workflow,mirror,invariants,workCalendar,telemetry,evidence]=await Promise.all([storageHealthV46(env),workflowHealthV51(env),mirrorHealthV52(env),runtimeInvariantHealthV55(env),workCalendarHealth(env),telemetryHealthV61(env),releaseEvidence(env,buildVersion,releaseFingerprint)]),capacityReady=!(storage?.capacity?.available===true&&storage.capacity.status==='critical'),blockers=[];
 ...
 body.storage_health=storage;body.workflow_health=workflow;body.mirror_health=mirror;body.runtime_invariants=invariants;body.work_calendar=workCalendar;body.telemetry_health=telemetry;body.runtime_blockers=blockers;body.runtime_ready=blockers.length===0;body.signoff_evidence=evidence;body.release_blockers=[...blockers,...(evidence.current?[]:['signoff_evidence'])];body.release_ready=body.runtime_ready&&evidence.current;return responseFrom(base,body);
}
