...
async function canonicalStart(req,env,cfg,kpi){
 ...
 const planId=cleanText(b.plan_id),plan=await one(env.DB,"SELECT * FROM entries WHERE id=? AND module='planning' AND deleted=0",planId);if(!plan)return out(req,env,{error:'Pilih planning yang telah dirilis PPIC'},409);const pp=J(plan.payload,{}),planCode=canonical(cfg,pp.machine);
 ...
 const material=cleanText(pp.material),plannedQty=Number(pp.target),shift=cleanText(pp.shift)||cleanText(b.shift)||null,group=cleanText(pp.group)||cleanText(b.group)||null;
 if(!material)return out(req,env,{error:'Material Planning Released belum tersedia'},409);if(!Number.isFinite(plannedQty)||plannedQty<=0)return out(req,env,{error:'Target Qty Planning Released tidak valid'},409);
 ...
 const mid=machineId(code);await run(env.DB,"INSERT INTO machine_registry(id,code,name,department,source_type,active,heartbeat_at) VALUES(?,?,?,'PROD','hmi-governed',1,?) ON CONFLICT(code) DO UPDATE SET active=1",mid,code,code,now());
 const st=await one(env.DB,'SELECT counter FROM machine_state WHERE machine_id=?',mid),id=uid(),ts=now();
 const reserved=await run(env.DB,"INSERT INTO production_runs(id,machine_id,pro,material,shift,group_name,operator_user_id,status,planned_qty,start_counter,start_ts,source,unit,plan_id) SELECT ?,?,?,?,?,?,?,'RUNNING',?,?,?,'hmi-governed',?,? WHERE NOT EXISTS(SELECT 1 FROM production_runs WHERE machine_id=? AND status='RUNNING')",id,mid,String(pp.pro),material,shift,group,u.id,plannedQty,Number(st?.counter||0),ts,unit,planId,mid);
 if(Number(reserved?.meta?.changes||0)!==1)return out(req,env,{error:'Mesin masih memiliki PRO aktif atau request Start PRO bersamaan sudah diproses'},409);
 await run(env.DB,"INSERT INTO machine_state(machine_id,state,pro,material,shift,group_name,counter,speed,since_ts,updated_ts,source) VALUES(?,?,?,?,?,?,0,0,?,?,?) ON CONFLICT(machine_id) DO UPDATE SET state='RUNNING',pro=excluded.pro,material=excluded.material,shift=excluded.shift,group_name=excluded.group_name,since_ts=excluded.since_ts,updated_ts=excluded.updated_ts,source=excluded.source",mid,'RUNNING',String(pp.pro),material,shift,group,ts,ts,'hmi-governed');
 await run(env.DB,"UPDATE entries SET payload=json_set(payload,'$.status','Dimulai'),version=version+1,updated=CURRENT_TIMESTAMP WHERE id=? AND module='planning'",planId);
 await run(env.DB,'INSERT INTO audit(id,user_id,action,entity_id,before_json,after_json) VALUES(?,?,?,?,?,?)',uid(),u.id,'SHOPFLOOR_START_CANONICAL',id,null,JSON.stringify({plan_id:planId,machine:code,source_machine:b.machine,planning_machine:pp.machine,pro:pp.pro,material,planned_qty:plannedQty,unit,shift,group}));
 return out(req,env,{ok:true,id,plan_id:planId,machine_code:code,source_machine:b.machine,canonicalized:cleanCode(b.machine)!==code,unit,material,planned_qty:plannedQty,shift,group});
}
