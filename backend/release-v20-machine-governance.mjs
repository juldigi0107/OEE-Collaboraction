...
async function canonicalStart(req,env,cfg,kpi){
 ...
 const reserved=await run(env.DB,"INSERT INTO production_runs(id,machine_id,pro,material,shift,group_name,operator_user_id,status,planned_qty,start_counter,start_ts,source,unit) SELECT ?,?,?,?,?,?,?,'RUNNING',?,?,?,'hmi-governed',? WHERE NOT EXISTS(SELECT 1 FROM production_runs WHERE machine_id=? AND status='RUNNING')",id,mid,String(pp.pro),material,shift,group,u.id,plannedQty,Number(st?.counter||0),ts,unit,mid);
 ...
 await run(env.DB,'INSERT INTO audit(id,user_id,action,entity_id,before_json,after_json) VALUES(?,?,?,?,?,?)',uid(),u.id,'SHOPFLOOR_START_CANONICAL',id,null,JSON.stringify({plan_id:b.plan_id,machine:code,source_machine:b.machine,planning_machine:pp.machine,pro:pp.pro,material,planned_qty:plannedQty,unit,shift,group}));
 return out(req,env,{ok:true,id,machine_code:code,source_machine:b.machine,canonicalized:cleanCode(b.machine)!==code,unit,material,planned_qty:plannedQty,shift,group});
}
