...
export async function handleRuntimeSignoffV54(req,env){
 ...
 try{
  const [workflow,mirror,invariants]=await Promise.all([workflowHealthV51(env),mirrorHealthV52(env),runtimeInvariantHealthV55(env)]),blockers=[];
  ...
  if(blockers.length)return out(req,env,{error:'Final UAT belum dapat dinyatakan Lulus karena runtime consistency belum hijau',blockers,workflow_health:workflow,mirror_health:mirror,runtime_invariants:invariants},409);
  return null;
