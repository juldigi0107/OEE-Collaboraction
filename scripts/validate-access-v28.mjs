...
const checks=[
 ...
 ['permissions business-facing',js.includes("create:'Tambah data'")&&js.includes("update:'Ubah data'")&&js.includes("delete:'Arsipkan data'")&&js.includes("config:'Konfigurasi'")],
 ['soft delete semantics explicit',js.includes('soft delete')&&governance.includes('soft delete')&&governance.includes('Arsipkan')],
...
const failed=checks.filter(([,ok])=>!ok);if(failed.length){for(const [n] of failed)console.error('FAIL:',n);process.exit(1);}console.log(`Access v28 validation OK — ${checks.length} account, archival, and permission presentation guards checked.`);
