...
const checks=[
 ...
 ['field projection resolves exact then unique normalized only',field82.includes('async function exactMachine')&&field82.includes("WHERE r.code=? AND r.active=1")&&field82.includes('matches.length===1')&&field82.includes("match:'ambiguous'")&&field82.includes("status:'machine_ambiguous'")],
 ['field display fails closed on ambiguous machine identity',field.includes("projection?.status==='machine_ambiguous'")&&field.includes('Perbaiki canonical machine / alias')],
 ...
 ['field display green state requires trusted exact-machine projection',field.includes("projection?.status==='machine_not_found'")&&field.includes("projection?.status==='machine_ambiguous'")&&field.includes("projection?.telemetry?.trusted===true")&&field.includes('telemetry belum authoritative')],
