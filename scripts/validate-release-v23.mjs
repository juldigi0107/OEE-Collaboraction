...
const checks=[
 ...
 ['production live mirror preserves NC and rule snapshot',live49.includes('nc:r.nc_qty')||live49.includes('nc,')&&live49.includes('hmi_quality_snapshot:true')&&live49.includes('quality_rule_label')],
 ...
 ['v81 contains no prototype language',!/\b(prototype|mockup|dummy|lorem ipsum|data demo)\b/i.test([hmi81,back81].join('\n'))]
];
const failed=checks.filter(([,ok])=>!ok);
if(failed.length){for(const [name] of failed)console.error('FAIL:',name);process.exit(1);}
console.log(`Release polish and HMI OEE parity validation OK — ${checks.length} UX, data-safety, NC, governance-snapshot, approval, mirror, and schema guards checked.`);
