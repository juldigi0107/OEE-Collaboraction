...
const checks=[
 ...
 ['latest run per planning authority',reconcile.includes('NOT EXISTS (')&&reconcile.includes('production_runs newer')&&reconcile.includes('newer.plan_id=r.plan_id')&&reconcile.includes('COALESCE(newer.end_ts,newer.start_ts)>COALESCE(r.end_ts,r.start_ts)')],
 ['mismatch priority prevents starvation',reconcile.includes('ORDER BY CASE')&&reconcile.includes("WHEN e.id IS NULL THEN 0")&&reconcile.includes("json_extract(e.payload,'$.verification_status')")&&reconcile.includes('ELSE 1 END')],
