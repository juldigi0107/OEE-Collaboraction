...
(()=>{
 const specs={sources:7,sheets:7,documents:4,record_chunks:9,entries:8,source_files:6,source_file_chunks:3,asset_catalog:3};
 ...
 const enc=new TextEncoder();let staged=[];
 ...
 const validB64=v=>typeof v==='string'&&v.length>0&&v.length%4===0&&/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(v);
 ...
 function validateBatch(obj,file,line){
  ...
  if(table==='source_file_chunks'&&Array.isArray(rows))rows.forEach((r,i)=>{if(!validB64(r?.[2]))issues.push({file,line,message:`Chunk file row ${i+1} bukan payload base64 yang valid.`});});
  const wire=JSON.stringify(obj??{}),size=enc.encode(wire).length;if(size>450000)issues.push({file,line,message:`Ukuran batch ${bytes(size)} melebihi batas backend 450 KB.`});
 ...
 async function stageFiles(files){
  ...
  if(files.length>250)summary.warnings.push({file:'Paket',message:'Jumlah file sangat banyak; proses impor dapat memerlukan waktu lebih lama.'});
  ...
  for(const file of files){
   ...
   for(let i=0;i<lines.length;i++){
    ...
    if(!v.issues.length){const item={file:file.name,line:i+1,table:v.table,rows:v.rows,size:v.size,obj:v.obj};staged.push(item);summary.batches++;summary.rows+=v.rows;summary.byTable[v.table]=(summary.byTable[v.table]||0)+v.rows;}
   }
 ...
 function renderReview(s){
  const host=$('#ic30Review');if(!host)return;const ok=s.issues.length===0&&s.batches>0;
 ...
 async function validateSelected(){
  const input=$('#ic30Files'),files=[...(input?.files||[])].sort((a,b)=>a.name.localeCompare(b.name)),button=$('#ic30Validate');if(button)button.disabled=true;$('#ic30Status').textContent='Membaca dan memvalidasi paket secara lokal…';
  try{const s=await stageFiles(files);renderReview(s);$('#ic30Status').textContent=s.issues.length?`${fmt(s.issues.length)} issue harus diperbaiki sebelum impor.`:`${fmt(s.batches)} batch siap ditinjau. D1 belum berubah.`;}catch(e){staged=[];$('#ic30Status').textContent='Validasi gagal: '+e.message;}finally{if(button)button.disabled=false;}
 }
 async function runImport(){
  ...
  const progress=$('#ic30Progress'),status=$('#ic30Status'),result=$('#ic30Result');progress.max=staged.length;progress.value=0;result.innerHTML='';const tableResult={},errors=[];let inserted=0,submitted=0;
  for(let i=0;i<staged.length;i++){
   const x=staged[i];status.textContent=`Mengimpor ${x.file} · batch ${i+1}/${staged.length} · ${labels[x.table]||x.table}`;
   try{const r=await api('/import-data','POST',x.obj),n=Number(r.inserted||0);inserted+=n;submitted+=x.rows;tableResult[x.table]??={submitted:0,inserted:0};tableResult[x.table].submitted+=x.rows;tableResult[x.table].inserted+=n;progress.value=i+1;}catch(e){errors.push({item:x,message:e.message});break;}
  }
  ...
  status.textContent=errors.length?'Impor dihentikan karena error backend.':'Impor selesai dan katalog sudah dimuat ulang.';$('#ic30Sources').onclick=()=>navigate('documents');$('#ic30Again').onclick=()=>navigate('import');if(errors.length){run.disabled=false;reset.disabled=false;validate.disabled=false;}
 }
 importCenter=function(){
  if(user?.role!=='superadmin')throw Error('Khusus Superadmin');staged=[];
  $('#content').innerHTML=heading('Pusat Impor Data','Validasi paket sumber terlebih dahulu sebelum menambahkan data ke D1')+`<div class="ic30-flow"><div class="active"><span>1</span><strong>Pilih paket</strong><small>JSONL hasil audit</small></div><div><span>2</span><strong>Validasi</strong><small>struktur & ukuran batch</small></div><div><span>3</span><strong>Tinjau</strong><small>komposisi & issue</small></div><div><span>4</span><strong>Impor</strong><small>INSERT OR IGNORE</small></div></div><section class="panel ic30-upload"><div><h2>Paket sumber terkontrol</h2><p>Pilih seluruh file <b>.jsonl</b> dari paket data yang sudah diaudit. Validasi pada tahap ini hanya membaca file di browser dan <b>belum mengubah D1</b>.</p><ul><li>Jenis tabel harus termasuk whitelist aplikasi.</li><li>Maksimal 20 row per batch dan 450 KB per request.</li><li>Chunk file diverifikasi sebagai base64 sebelum dikirim.</li><li>ID yang sudah ada akan dilewati; data existing tidak dihapus oleh endpoint impor.</li></ul></div><label class="ic30-file"><span>Pilih file JSONL</span><input type="file" id="ic30Files" multiple accept=".jsonl,application/json"><small>File tetap berada di perangkat sampai Anda menjalankan impor.</small></label><button id="ic30Validate" class="primary">Validasi Paket</button><div id="ic30Status" class="muted" role="status">Menunggu paket.</div><progress id="ic30Progress" max="1" value="0"></progress></section><section class="panel" id="ic30Review"><div class="ic30-empty">Pilih paket JSONL lalu tekan Validasi Paket. Tidak ada perubahan D1 sebelum tahap eksekusi.</div></section><section id="ic30Result"></section>`;$('#ic30Validate').onclick=validateSelected;
 };
})();
