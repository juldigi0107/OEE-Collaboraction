...
(()=>{
 ...
 const domainMap={production:'production',downtime:'production',checklist:'production',logbook:'production',process:'quality',energy:'production',batch:'production',quality:'quality',maintenance:'maintenance',confirmation:'ppic',planning:'ppic',development:'development',master:'master',project:'master'};
 const domainLabel={production:'Production / OEE',quality:'Quality / Reject & Process',maintenance:'Maintenance / Breakdown',ppic:'PPIC / Planning & Confirmation',development:'PDS / Development',master:'Master / Project'};
 const validDate=v=>/^\d{4}-\d{2}-\d{2}$/.test(String(v||''))&&!Number.isNaN(Date.parse(String(v)));
 const uniq=a=>[...new Set(a.filter(Boolean))];
 const dateOf=p=>{for(const k of ['work_date','date','report_date','finish_date','start_date'])if(validDate(p?.[k]))return String(p[k]);for(const k of ['created_ts','start_ts','end_ts','requested_ts','closed_ts']){const v=p?.[k];if(v&&!Number.isNaN(Date.parse(v)))return new Date(v).toISOString().slice(0,10);}return '';};
 function sourceAuthority(){const domain=domainMap[opModule],cfg=window.DG16?DG16.read(DG16.keys.sources):{},configured=!!(window.DG16&&DG16.approved(cfg)),raw=cfg?.domains?.[domain],id=typeof raw==='string'?raw:raw?.source_id,src=(catalog?.sources||[]).find(s=>s.id===id);return {domain,configured,id:id||'',name:src?.name||raw?.source_name||'',exists:!!src,approved:configured&&!!id&&!!src};}
 function notes(list){const out=[];
  ...
  if(opModule==='maintenance')out.push('Periode maintenance mengikuti tanggal kerja/transaksi; timestamp hanya menjadi fallback dan dapat melintasi akhir bulan.');
  ...
  if(opModule==='process')out.push('Process parameter berada pada domain Quality; evaluasi batas LSL/USL tidak digabungkan dengan output Production.');
  return out;
 }
 ...
 function paintDataContext(){if(!Array.isArray(rows)||!opModule)return;const table=document.querySelector('#content .module-table-v25');if(!table)return;document.querySelector('#content .data-context-v32')?.remove();const list=rows.map(payload),auth=sourceAuthority(),dates=list.map(dateOf).filter(Boolean).sort(),historical=list.filter(p=>p.source_sheet||p.source_record).length,live=list.length-historical;
  const box=document.createElement('section');box.className='panel data-context-v32';const head=document.createElement('div');head.className='release-section-head';const left=document.createElement('div');add(left,'span','DATA CONTEXT','eyebrow');add(left,'h3',domainLabel[auth.domain]||modules[opModule]?.[0]||opModule);add(left,'p','Periode mengikuti tanggal transaksi/tanggal kerja; timestamp operasional hanya fallback. Nama file tidak digunakan sebagai periode laporan.');head.append(left);const authText=auth.approved?`Authoritative · ${auth.name}`:auth.configured&&auth.id&&!auth.exists?'Authority source tidak ditemukan':'Source authority belum disahkan';add(head,'span',authText,'release-status '+(auth.approved?'ok':'warn'));box.append(head);
 ...
 window.DataContextV32={paint:paintDataContext,sourceAuthority,notes,dateOf};
})();
