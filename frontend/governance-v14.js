...
(()=>{
  ...
  const actionText=a=>String(a||'Aktivitas').replace(/[_-]+/g,' ').replace(/\b\w/g,c=>c.toUpperCase()).replace(/Create/g,'Tambah').replace(/Update/g,'Ubah').replace(/Delete/g,'Hapus').replace(/Login/g,'Login').replace(/Logout/g,'Logout');
  ...
  userForm=function(u={}){
    ...
    const sheet=document.querySelector('#userForm .sheetinfo');if(sheet)sheet.textContent='Admin hanya dapat mengubah data pada department sendiri sesuai izin. Viewer selalu read-only. Perubahan akun mengakhiri sesi akun terkait untuk keamanan.';
  };

  audit=async function(){
    const data=await api('/audit'),actors=new Set(data.map(x=>x.user_id||x.name).filter(Boolean)),today=new Date().toISOString().slice(0,10),todayCount=data.filter(x=>String(x.created||'').slice(0,10)===today).length,destructive=data.filter(x=>/delete|hapus/i.test(x.action||'')).length;
    $('#content').innerHTML=heading('Audit Log','Jejak perubahan dan aktivitas administratif yang dapat ditelusuri')+
      `<div class="release-kpis"><div><span>Aktivitas ditampilkan</span><strong>${fmt(data.length)}</strong><small>maksimum 200 terbaru</small></div><div><span>Pengguna terkait</span><strong>${fmt(actors.size)}</strong><small>berdasarkan audit aktif</small></div><div><span>Aktivitas hari ini</span><strong>${fmt(todayCount)}</strong><small>mengikuti timestamp server</small></div><div><span>Aksi penghapusan</span><strong>${fmt(destructive)}</strong><small>tetap memiliki jejak audit</small></div></div>`+
      `<section class="panel"><div class="release-section-head"><div><h2>Riwayat aktivitas</h2><p>Nilai sensitif disamarkan pada tampilan ini.</p></div></div><div class="tablewrap"><table class="release-table audit-release-table"><thead><tr><th>Waktu</th><th>Pengguna</th><th>Aktivitas</th><th>Objek</th><th>Detail</th></tr></thead><tbody>${data.map((a,i)=>`<tr><td>${esc(new Date(a.created).toLocaleString('id-ID'))}</td><td><strong>${esc(a.name||'System')}</strong></td><td>${esc(actionText(a.action))}</td><td>${esc(a.entity_id||'—')}</td><td><details><summary>Lihat perubahan</summary><div class="audit-diff"><div><span>Sebelum</span><pre>${esc(redact(a.before_json))}</pre></div><div><span>Sesudah</span><pre>${esc(redact(a.after_json))}</pre></div></div></details></td></tr>`).join('')||'<tr><td colspan="5"><div class="release-empty">Belum ada aktivitas yang tercatat.</div></td></tr>'}</tbody></table></div></section>`;
  };
