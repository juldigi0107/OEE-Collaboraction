...
(()=>{
  ...
  async function approvalWorkspace(){
    ...
    $('#content').innerHTML=heading('Approval & Verifikasi','Antrean verifikasi hasil produksi, downtime, dan quality sesuai kewenangan department')+
      ...
      `<section class="panel"><div class="release-section-head"><div><h2>Menunggu keputusan</h2><p>Request dibuat otomatis setelah event operasional selesai. Satuan kuantitas dipertahankan dari event sumber.</p></div>${pending.length?`<span class="release-status warn">${pending.length} pending</span>`:''}</div>${approvalTable(pending,true)}</section>`+
  ...
  function approvalTable(rows,actionable){
    ...
    return `<div class="tablewrap"><table class="release-table"><thead><tr><th>Waktu</th><th>Jenis</th><th>Mesin / PRO</th><th>Ringkasan</th><th>Requester</th><th>Status</th>${actionable?'<th>Keputusan</th>':''}</tr></thead><tbody>${rows.map(a=>{const e=a.entity||{},unit=String(e.unit||'').trim(),u=unit?' '+esc(unit):'',legacy=!unit&&(a.entity_type==='production_run'||a.entity_type==='quality'),legacyNote=legacy?'<small>Satuan event lama belum tercatat · jangan digabungkan lintas unit</small>':'';const summary=a.entity_type==='production_run'?`Actual ${fmt(Number(e.actual_qty||0))}${u} · Good ${fmt(Number(e.good_qty||0))}${u} · Reject ${fmt(Number(e.reject_qty||0))}${u}${legacyNote}`:a.entity_type==='downtime'?`${esc(e.class||'')} · ${esc(e.code||e.reason||'')} ${e.root_cause?`<small>Root cause: ${esc(e.root_cause)}</small>`:''}`:`Sample ${fmt(Number(e.sample_qty||0))}${u} · Reject ${fmt(Number(e.reject_qty||0))}${u} · ${esc(e.decision||'')}${legacyNote}`;const canDecide=a.status==='PENDING'&&(user.role==='superadmin'||can(approvalDept(a.entity_type),'update'));return `<tr><td>${esc(new Date(a.requested_ts).toLocaleString('id-ID'))}<small>${esc(approvalStep(a.step))}</small></td><td><strong>${esc(approvalType(a.entity_type))}</strong></td><td>${esc(e.machine_name||e.machine||'—')}<small>${esc(e.pro||'')}</small></td><td>${summary}</td><td>${esc(a.requested_by_name||'System / user')} ${a.note?`<small>${esc(a.note)}</small>`:''}</td><td>${labelStatus(a.status)}</td>${actionable?`<td>${canDecide?`<div class="approval-actions"><button class="approve" data-approval="${esc(a.id)}" data-decision="APPROVED">Setujui</button><button class="reject" data-approval="${esc(a.id)}" data-decision="REJECTED">Tolak</button></div>`:'<small>Hanya lihat</small>'}</td>`:''}</tr>`;}).join('')}</tbody></table></div>`;
  }
  ...
  async function enhanceOperationalClosure(){
    ...
    if(call.status==='ACKNOWLEDGED'&&can('MTC','update')&&!card.querySelector('#closeMtc')){
      const btn=document.createElement('button');btn.id='closeMtc';btn.className='primary';btn.textContent='Tutup Maintenance';card.append(btn);btn.onclick=()=>closeMaintenanceDialog(call);
    }
  }
