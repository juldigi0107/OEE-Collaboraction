...
(()=>{
 ...
 const help={
  ...
  development:'Gunakan durasi dan biaya aktual. Mata uang dicatat eksplisit dan tidak diasumsikan dari nama file atau label lama.',
 ...
 function developmentCurrency(form,row){
  if(opModule!=='development'||field(form,'currency'))return;const cost=field(form,'cost');if(!cost)return;const p=rowPayload(row),costLabel=cost.closest('label');if(costLabel){const first=[...costLabel.childNodes].find(n=>n.nodeType===3&&String(n.textContent||'').trim());if(first)first.textContent='Biaya aktual';}
 ...
 function enhance(row){const form=$('#entryForm');if(!form||form.dataset.v38)return;form.dataset.v38='1';sourceContext(form,row);checklistControls(form);planningUnit(form,row);developmentCurrency(form,row);markRequired(form,row);unitNotice(form);signedNotice(form);statusContext(form);productionPreview(form);const save=form.querySelector('.formactions .primary');if(save)save.textContent=row?'Simpan perubahan':'Simpan transaksi';}
